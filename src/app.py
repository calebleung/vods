    ...
    def post(self):
        ...
        with sqlite3.connect(config['DB']['name']) as conn:
            ...
            for i, game in enumerate(games):
                for row in c.execute('SELECT :gameIndex, played.vod_id, played.start_at, vods.title, vods.created_at FROM played INNER JOIN vods ON vods.vod_id = played.vod_id WHERE game_id=:gameID', {'gameIndex': str(i), 'gameID': game[0]}):
